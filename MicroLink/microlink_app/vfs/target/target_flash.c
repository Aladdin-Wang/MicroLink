...
static error_t target_flash_host_init(void);
static error_t target_flash_host_uninit(void);
...
static const flash_intf_t flash_intf = {
    target_flash_host_init,
    target_flash_host_uninit,
...
static uint32_t target_flash_erase_sector_size(uint32_t addr)
{
    if (g_board_info.target_cfg){
        if(g_board_info.target_cfg->sector_info_length > 0) {
            ...
            uint32_t wSectorEnd = g_board_info.target_cfg->flash_dev_info->sectors[chPartitionIndex + 1].AddrSector;
            ...
            for(wSector = 0; wSector < 0XFFFF; wSector++) {
                ...
                if(wSectorStart >= wSectorEnd && wSectorStart < g_board_info.target_cfg->flash_dev_info->szDev) {
                    chPartitionIndex++;
                    wSectorEnd = g_board_info.target_cfg->flash_dev_info->sectors[chPartitionIndex + 1].AddrSector;
                    wSectorStart = g_board_info.target_cfg->flash_dev_info->sectors[chPartitionIndex].AddrSector;
                    wSectorSize = g_board_info.target_cfg->flash_dev_info->sectors[chPartitionIndex].szSector;
                }
                if (wSectorEnd == 0xFFFFFFFF && wSectorStart >= g_board_info.target_cfg->flash_dev_info->szDev) {
                    break;
                }
            }
        }
