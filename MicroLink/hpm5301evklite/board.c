...
void bootloader_init_console(void)
{
     ...
     * And a uart rx dma request will be generated by default uart fifo dma trigger level.
     */
    init_uart_pins((UART_Type *) BOARD_BOOT_UART_BASE);

    clock_add_to_group(BOARD_BOOT_UART_CLK_NAME, 0);

    cfg.type = BOARD_CONSOLE_TYPE;
    cfg.base = (uint32_t)BOARD_BOOT_UART_BASE;
    cfg.src_freq_in_hz = clock_get_frequency(BOARD_BOOT_UART_CLK_NAME);
    cfg.baudrate = BOARD_BOOT_UART_BAUDRATE;
